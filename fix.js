...
const parallaxIndex = scriptContent.indexOf('/* ── Parallax Hero ───────────────────────────────────── */');
if (parallaxIndex !== -1) {
    scriptContent = scriptContent.substring(0, parallaxIndex);
}

const newScriptContent = `/* ── Parallax Hero ───────────────────────────────────── */
...
// Replacing literal escaped characters so it outputs template strings properly
const actualNewScriptContent = newScriptContent.replace(/\\\\`/g, '`').replace(/\\\\\$/g, '$');

fs.writeFileSync('script.js', scriptContent + actualNewScriptContent, 'utf-8');

let styleContent = fs.readFileSync('style.css', 'utf-8');
styleContent = styleContent.replace(/--card-phase:.*?;/g, '');
styleContent = styleContent.replace(/--card-pos:.*?;/g, '');
